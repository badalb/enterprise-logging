...
import com.test.aeron.RateReporter;
import com.test.aeron.SampleConfiguration;
...
public class StreamingPublisher {
	...
	private static final UnsafeBuffer ATOMIC_BUFFER = new UnsafeBuffer(
			ByteBuffer.allocateDirect(MESSAGE_LENGTH));
	private static final BusySpinIdleStrategy OFFER_IDLE_STRATEGY = new BusySpinIdleStrategy();
	private static final IntSupplier LENGTH_GENERATOR = composeLengthGenerator(
			RANDOM_MESSAGE_LENGTH, MESSAGE_LENGTH);
	...
	public static void main(final String[] args) throws Exception {
		if (MESSAGE_LENGTH < SIZE_OF_LONG) {
			throw new IllegalArgumentException(String.format(
					"Message length must be at least %d bytes", SIZE_OF_LONG));
		}

		final MediaDriver driver = EMBEDDED_MEDIA_DRIVER ? MediaDriver
				.launchEmbedded() : null;
		...
		final RateReporter reporter = new RateReporter(
				TimeUnit.SECONDS.toNanos(1), StreamingPublisher::printRate);
		final ExecutorService executor = Executors.newFixedThreadPool(100);

		executor.execute(reporter);

		// Connect to media driver and add publication to send messages on the
		// configured channel and stream ID.
		...
		try (final Aeron aeron = Aeron.connect(context);
				final Publication publication = aeron.addPublication(CHANNEL,
						STREAM_ID)) {
			final ContinueBarrier barrier = new ContinueBarrier(
					"Execute again?");
			...
			do {
				...
				System.out
						.format("\nStreaming %,d messages of%s size %d bytes to %s on stream Id %d\n",
								...
								MESSAGE_LENGTH, CHANNEL, STREAM_ID);
				...
				for (long i = 0; i < NUMBER_OF_MESSAGES; i++) {
					...
					while (publication.offer(ATOMIC_BUFFER, 0, length) < 0L) {
						// The offer failed, which is usually due to the
						// publication
						// being temporarily blocked. Retry the offer after a
						// short
						// spin/yield/sleep, depending on the chosen
						// IdleStrategy.
						backPressureCount++;
				...
				if (0 < LINGER_TIMEOUT_MS) {
					System.out.println("Lingering for " + LINGER_TIMEOUT_MS
							+ " milliseconds...");
	...
	public static void printRate(final double messagesPerSec,
			final double bytesPerSec, final long totalFragments,
			final long totalBytes) {
		if (printingActive) {
			System.out
					.format("%.02g msgs/sec, %.02g bytes/sec, totals %d messages %d MB\n",
							messagesPerSec, bytesPerSec, totalFragments,
							totalBytes / (1024 * 1024));
		}
	}

	private static IntSupplier composeLengthGenerator(final boolean random,
			final int max) {
		if (random) {
