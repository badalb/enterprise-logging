...
public class RateReporter implements Runnable {
	...
	/**
	 * Create a rate reporter with the given report interval in nanoseconds and
	 * the reporting function.
	 *
	 * @param reportInterval
	 *            in nanoseconds
	 * @param reportingFunc
	 *            to call for reporting rates
	 */
	...
	public void run() {
		do {
			...
			final double messagesPerSec = ((currentTotalMessages - lastTotalMessages) * reportIntervalNs)
					/ (double) timeSpanNs;
			final double bytesPerSec = ((currentTotalBytes - lastTotalBytes) * reportIntervalNs)
					/ (double) timeSpanNs;

			reportingFunc.onReport(messagesPerSec, bytesPerSec,
					currentTotalMessages, currentTotalBytes);
